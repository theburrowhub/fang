...
fn entry_style(entry: &FileEntry) -> (&'static str, Style) {
    if entry.is_dir { ("\u{25B6} ", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)) }
    else if entry.is_symlink { ("\u{21AA} ", Style::default().fg(Color::Cyan).add_modifier(Modifier::ITALIC)) }
    else if entry.is_executable { ("\u{26A1} ", Style::default().fg(Color::Green)) }
    else { ("\u{00B7} ", Style::default().fg(Color::White)) }
}

pub fn render(frame: &mut Frame, area: Rect, state: &AppState) {
    let border_style = panel_border_style(state.focused_panel == FocusedPanel::FileList);
    let dir_name = state.current_dir.file_name().and_then(|n| n.to_str()).unwrap_or("/");
    let title = if state.search_query().is_some() {
        format!(" {} [{}/{}] ", dir_name, state.visible_count(), state.entries.len())
    } else {
    ...
    let items: Vec<ListItem> = visible.iter().map(|entry| {
        ...
        let size_str = if entry.is_dir { "  dir".to_string() } else { format_size_compact(entry.size) };
        let padded_name = format!("{:<width$}", name, width = name_width);
        let line = Line::from(vec![
            Span::styled(icon, style),
            Span::styled(padded_name, style),
            Span::styled(format!("{:>5}", size_str), Style::default().fg(Color::DarkGray)),
        ]);
        ListItem::new(line)
    }).collect();
    let highlight_style = Style::default().bg(Color::DarkGray).add_modifier(Modifier::BOLD);
    let list = List::new(items).block(block).highlight_style(highlight_style).highlight_symbol("> ");
    let mut list_state = ListState::default();
    if !visible.is_empty() { list_state.select(Some(state.selected_index)); }
    frame.render_stateful_widget(list, area, &mut list_state);
    if let AppMode::Search { query } = &state.mode {
        let search_area = Rect { x: area.x + 1, y: area.y + area.height.saturating_sub(2), width: area.width.saturating_sub(2), height: 1 };
