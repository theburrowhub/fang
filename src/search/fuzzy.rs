...
///
/// Delegates to `AppState::visible_entries` so the filtering logic lives in
/// exactly one place.
pub fn visible_entries<'a>(state: &'a AppState) -> Vec<(usize, &'a FileEntry)> {
    state.visible_entries()
}

/// Returns the index inside `state.entries` of the currently selected entry,
/// or `None` when there are no visible entries.
///
/// Uses an iterator instead of collecting a full `Vec` so only one element is
/// ever accessed regardless of how many entries exist.
...
/// Returns a reference to the currently selected `FileEntry`, or `None` when
